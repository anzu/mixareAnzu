...
public class Json extends DataHandler {
			...
			if (dataArray != null) {
				...
				for (int i = 0; i < top; i++) {					
					...
					switch(dataformat) {
						...
						case UBIQUITY: ma = processUbiquityJSONObject(jo); break;
	...
	public Marker processUbiquityJSONObject(JSONObject jo) throws JSONException, NumberFormatException {
		Marker ma = null;
		if(jo.has("geometry")) {
			try {
				Log.d(MixView.TAG, "Json.java processing Ubiquity JSON data");
				
				String link=null;
				
				// Parse the JSON object properly
				
				String geometry = jo.getString("geometry");		
				geometry = geometry.replaceAll("<Point><coordinates>", "");
				geometry = geometry.replaceAll("<\\/coordinates><\\/Point>", "");
				geometry = geometry.replaceAll("<LineString><coordinates>", "");
				geometry = geometry.replaceAll("<\\/coordinates><\\/LineString>", "");
				
				String category = jo.getString("category");
				String title = jo.getString("title");
				
				DATASOURCE ubiquityDataSource = null;
				
				if (category.equalsIgnoreCase("Events"))
					ubiquityDataSource = DataSource.DATASOURCE.EVENTS;
				else if (category.equalsIgnoreCase("Merchandise"))
					ubiquityDataSource = DataSource.DATASOURCE.MERCHANDISE;
				else if (category.equalsIgnoreCase("Refreshments"))
					ubiquityDataSource = DataSource.DATASOURCE.REFRESHMENTS;
				else if (category.equalsIgnoreCase("Restrooms"))
					ubiquityDataSource = DataSource.DATASOURCE.RESTROOMS;
				else if (category.equalsIgnoreCase("Social"))
					ubiquityDataSource = DataSource.DATASOURCE.SOCIAL;
				
				if (title.contains("Path")) {
					//nothing - do not create marker!
				}
				else {
				   ma = new POIMarker(title,
						   		Double.valueOf(geometry.split(",")[1]), // latitude 
						   		Double.valueOf(geometry.split(",")[0]), // longitude
						   		0, // altitude Double.valueOf(geometry.split(",")[2])
						   		link,
						   		ubiquityDataSource); 
					Log.i(MixView.TAG, "Json // " + jo.getString("title") + " // Category: " + category);
				}
			}
			catch (Exception e) {
				Log.e(MixView.TAG, e.getMessage());
				e.printStackTrace();
			}
		}
		return ma;
